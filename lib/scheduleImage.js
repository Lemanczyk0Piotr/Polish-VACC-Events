...
export function renderScheduleImage(event, assignments, remarks = '') {
  ...
  // dopiero axisStart/axisEnd poniżej). Używane niżej do wykrycia przerwy
  // przed pierwszą/po ostatniej zmianie na pozycji (kafelek "FREE" — ta sama
  // logika co components/ScheduleGrid.js, żeby obrazek wysyłany na Discorda
  // wyglądał identycznie jak podgląd na stronie).
  ...
  for (const group of groups) {
    ...
    for (const p of group.positions) {
      ...
      // Przerwa przed pierwszą i/albo po ostatniej zmianie na tej pozycji
      // względem CAŁEGO czasu eventu (nie osi, która i tak się rozciąga) —
      // tam rysujemy szary kafelek "FREE", tak samo jak na podglądzie na
      // stronie (components/ScheduleGrid.js). Luki MIĘDZY zmianami zostają
      // puste, bez kafelka.
      const firstStart = new Date(sorted[0].time_start);
      const lastEnd = new Date(sorted[sorted.length - 1].time_end);
      const hasLeadingFree = firstStart.getTime() > eventStart.getTime();
      const hasTrailingFree = lastEnd.getTime() < eventEnd.getTime();
      ...
      sorted.forEach((a, idx) => {
        ...
        const x1 = xOf(s) + (touchesPrev || (idx === 0 && hasLeadingFree) ? GAP / 2 : 0);
        const x2 = xOf(e) - (touchesNext || (idx === sorted.length - 1 && hasTrailingFree) ? GAP / 2 : 0);
        const w = Math.max(2, x2 - x1);

        // Prawdziwy wiersz "- - - FREE - - -" (a.is_free) — administrator
        // wybrał go świadomie zamiast kontrolera, więc dostaje ten sam
        // szary/kreskowany wygląd co auto-wykryte przerwy na brzegach
        // (freeSegments niżej), tylko że może stać w DOWOLNYM miejscu —
        // nawet między dwiema prawdziwymi zmianami.
        if (a.is_free) {
          ...
          const freeLabelWidth = ctx.measureText(freeLabel).width;
          if (freeLabelWidth + 12 < w) {
            ctx.fillStyle = c.mutedDim;
            ctx.fillText(freeLabel, x1 + (w - freeLabelWidth) / 2, y + ROW_H / 2);
          }
          return;
        }
      ...
      const freeSegments = [];
      if (hasLeadingFree) freeSegments.push({ start: eventStart, end: firstStart, insetRight: true });
      if (hasTrailingFree) freeSegments.push({ start: lastEnd, end: eventEnd, insetLeft: true });
      for (const seg of freeSegments) {
        const fx1 = xOf(seg.start) + (seg.insetLeft ? GAP / 2 : 0);
        const fx2 = xOf(seg.end) - (seg.insetRight ? GAP / 2 : 0);
        const fw = Math.max(2, fx2 - fx1);

        ctx.fillStyle = c.border;
        roundRect(ctx, fx1, y + 3, fw, ROW_H - 6, 5);
        ctx.fill();
        ctx.save();
        ctx.setLineDash([4, 3]);
        ctx.strokeStyle = c.mutedDim;
        ...
        if (labelWidth + 12 < fw) {
          ctx.fillStyle = c.mutedDim;
          ctx.fillText(label, fx1 + (fw - labelWidth) / 2, y + ROW_H / 2);
        }
      }
      ctx.restore();
