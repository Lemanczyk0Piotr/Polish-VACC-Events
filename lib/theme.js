...
// announcement=cyjan). Bez `label` — teksty badge'y idą teraz przez
// t('events.kindLabel.<klucz>') / t('events.statusLabel.<klucz>') w lib/i18n.js,
// żeby się tłumaczyły z językiem (2026-09-03: były wcześniej hardkodowane
// po angielsku, więc strona Events nie różniła się wcale między PL i EN).
export const eventKindMeta = {
  event: { color: colors.red, bg: colors.redBg },
  exam: { color: colors.purple, bg: colors.purpleBg },
  announcement: { color: colors.cyan, bg: colors.cyanBg },
};

export const eventStatusMeta = {
  draft: { color: colors.muted },
  published: { color: colors.green },
  completed: { color: colors.mutedDim },
};
...
export const shared = {
  ...
  input: {
    // border-box so `width: '100%'` + padding never pushes the element past
    // its container (was causing textareas/inputs to overflow their card by
    // the padding amount).
...
// Endorsementy mają w tym projekcie TRZY warstwy nazw — warto je rozróżniać:
//
//  1. w bazie i w PLVACC:  S2-PE, S2-ME, S3-ME, C1-ME
//     Synchronizacja (lib/rosterSync.js -> endorsement_info) zapisuje dokładnie
//     to, co zwraca API, więc kolumna `endorsements` jest wierna źródłu.
//  2. stare etykiety aplikacji: PE, S2-CE, S3-CE, C1-CE
//     Sprzed migracji `rename_endorsements_to_plvacc_names`; tłumaczone na
//     postać z punktu 1 przy odczycie (ENDORSEMENT_ALIASES).
//  3. na ekranie: S2-PE, S2-CE, S3-CE, C1-CE
//     User woli nazewnictwo z "CE" (prośba z 2026-09-18), więc tłumaczymy je
//     dopiero przy wyświetlaniu (ENDORSEMENT_DISPLAY). Baza zostaje przy
//     nazwach PLVACC — inaczej każdy sync musiałby przepisywać je w tę i z
//     powrotem, a przy zmianie nazw w API rozjechałoby się jedno z drugim.
//
// Zasady wyświetlania: pokazujemy tylko to, co kontroler faktycznie ma; ME dla
// S2 jest ukrywane, gdy jest ME dla S3 (wtedy z niego wynika); S2 świeci na
// czerwono u kontrolera z ratingiem S3 (powinien mieć wyższy), S3 świeci na
// czerwono u C1 bez C1 (powinien podbić); C1 i PE nigdy nie świecą.
// Warstwa 2 -> 1. Zostaje na stałe: baza i wdrożony kod nigdy nie zmieniają
// się w tej samej sekundzie, a bez tego wersja aplikacji sprzed migracji
// przestaje pokazywać jakiekolwiek plakietki (dokładnie to się stało
// 2026-09-18).
const ENDORSEMENT_ALIASES = {
  PE: 'S2-PE',
  'S2-CE': 'S2-ME',
  'S3-CE': 'S3-ME',
  'C1-CE': 'C1-ME',
...
// Warstwa 1 -> 3: co widzi user. Zmiana TYLKO kosmetyczna — wartości w bazie
// zostają w postaci PLVACC.
const ENDORSEMENT_DISPLAY = {
  'S2-ME': 'S2-CE',
  'S3-ME': 'S3-CE',
  'C1-ME': 'C1-CE',
};

export function endorsementLabel(value) {
  const canonical = ENDORSEMENT_ALIASES[value] || value;
  return ENDORSEMENT_DISPLAY[canonical] || canonical;
}

export function endorsementBadges(c) {
  const e = (c?.endorsements || []).map((x) => ENDORSEMENT_ALIASES[x] || x);
  const has = (x) => e.includes(x);
  const badges = [];
  if (has('S2-PE')) badges.push({ label: endorsementLabel('S2-PE'), color: colors.purple, bg: colors.purpleBg });
  if (has('S3-ME')) {
    const warn = c.rating === 'C1' && !has('C1-ME');
    badges.push({ label: endorsementLabel('S3-ME'), color: warn ? colors.red : colors.gold, bg: warn ? colors.redBg : colors.goldBg });
  } else if (has('S2-ME')) {
    const warn = c.rating === 'S3';
    badges.push({ label: endorsementLabel('S2-ME'), color: warn ? colors.red : colors.gold, bg: warn ? colors.redBg : colors.goldBg });
  }
  if (has('C1-ME')) badges.push({ label: endorsementLabel('C1-ME'), color: colors.gold, bg: colors.goldBg });
  return badges;
}
