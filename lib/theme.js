...
// Plakietki S2-PE / S2-ME / S3-ME / C1-ME z kolumny controllers.endorsements
// (text[]). Nazwy są takie same jak w PLVACC, bo od 2026-09-18 synchronizacja
// (lib/rosterSync.js -> endorsement_info) zapisuje je żywcem z API — wcześniej
// aplikacja miała własne etykiety PE/S2-CE/S3-CE/C1-CE i nic ich nie
// aktualizowało.
//
// Zasady wyświetlania: pokazujemy tylko to, co kontroler faktycznie ma; S2-ME
// jest ukrywane, gdy jest S3-ME (wtedy wynika z niego); S2-ME świeci na
// czerwono u kontrolera z ratingiem S3 (powinien mieć S3-ME); S3-ME świeci na
// czerwono u C1 bez C1-ME (powinien podbić); C1-ME i S2-PE nigdy nie świecą.
// Stare etykiety aplikacji -> nazwy PLVACC. Migracja
// `rename_endorsements_to_plvacc_names` przepisała bazę, ale tłumaczenie
// zostaje TUTAJ na stałe: baza i wdrożony kod nigdy nie zmieniają się w tej
// samej sekundzie, a bez tego wersja aplikacji sprzed migracji przestaje
// pokazywać jakiekolwiek plakietki (dokładnie to się stało 2026-09-18).
// Dzięki temu obie nazwy renderują się poprawnie, niezależnie od kolejności.
...
export function endorsementBadges(c) {
  ...
  const badges = [];
  if (has('S2-PE')) badges.push({ label: 'S2-PE', color: colors.purple, bg: colors.purpleBg });
  if (has('S3-ME')) {
    const warn = c.rating === 'C1' && !has('C1-ME');
    badges.push({ label: 'S3-ME', color: warn ? colors.red : colors.gold, bg: warn ? colors.redBg : colors.goldBg });
  } else if (has('S2-ME')) {
    const warn = c.rating === 'S3';
    badges.push({ label: 'S2-ME', color: warn ? colors.red : colors.gold, bg: warn ? colors.redBg : colors.goldBg });
  }
  if (has('C1-ME')) badges.push({ label: 'C1-ME', color: colors.gold, bg: colors.goldBg });
  return badges;
}
