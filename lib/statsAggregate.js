...
// Podpisy kontrolerów (i cała reguła „bez admina tylko CID") mieszkają w
// lib/identity.js — tu tylko re-eksport, żeby strony statystyk nie musiały
// importować z dwóch miejsc.
export { controllerLabel, controllerName } from './identity';
...
export function aggregateStats(assignments, eventsById = null) {
  ...
  for (const a of assignments || []) {
    // Wiersz "- - - FREE - - -" (a.controllers === null — świadomie wybrana
    // opcja zamiast prawdziwego kontrolera, patrz pages/events/[id].js) NIE
    // jest zmianą kogokolwiek, więc pomijamy go całkowicie w statystykach —
    // ani czas, ani liczba zmian, ani obsada pozycji/typu/eventu nie
    // powinny rosnąć o czas, w którym jawnie nikt nie kontrolował. Ten sam
    // wzorzec stosują już /top-controllers i sendMonthlySummary
    // (lib/discordDispatch.js): `if (!r.controllers) continue`.
...
export function shiftsByPosition(assignments) {
  ...
  for (const a of assignments || []) {
    ...
    map.get(callsign).items.push({
      ...
      // Wiersz "- - - FREE - - -" zostaje na liście (żeby admin widział, że
      // ten przedział świadomie stoi pusty), ale jego "minuty" nie wliczają
      // się do sumy p.minutes niżej — to nie jest czas realnie kontrolowany.
      isFree: !a.controllers,
    });
  }
  return Array.from(map.values())
    .map((p) => {
      p.items.sort((x, y) => String(x.from).localeCompare(String(y.from)));
      p.minutes = p.items.reduce((acc, i) => acc + (i.isFree ? 0 : i.minutes), 0);
      return p;
    })
