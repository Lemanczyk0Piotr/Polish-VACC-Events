...
const EVENT_SELECT =
  'id, title, event_date, time_start, time_end, kind, category, status, image_url, notes, external_link, statsim_url, created_at';
...
// Rozpiska obsady — buduje listę pozycji z przypisanymi zmianami.
export async function sendSchedule(supabase, eventId, { force = false } = {}) {
  const event = await getEvent(supabase, eventId);
  const { data: rows, error } = await supabase
    .from('event_assignments')
    .select(
      'id, time_start, time_end, positions(callsign, type), controllers:controllers!event_assignments_controller_id_fkey(name, cid), student:controllers!event_assignments_student_id_fkey(name, cid)'
    )
    .eq('event_id', eventId);
  if (error) throw new Error(error.message);
    ...
    .map((p) => {
      ...
      return {
        ...
        lines: p.items.map((it) => {
          const who = it.controllers?.name || it.controllers?.cid || '—';
          const student = it.student ? ` + ${it.student.name || it.student.cid} (uczeń)` : '';
          return `\`${hhmm(it.time_start)}–${hhmm(it.time_end)}\` ${who}${student}`;
        }),
      };
    });
  ...
  return sendOnce({
    ...
    payload: {
      mentionRole: true,
      content: [
        `🗒️ **Rozpiska obsady — ${event.title}**`,
        url ? `Pełny harmonogram: ${url}` : null,
      ]
        .filter(Boolean)
        .join('\n'),
      embeds: buildScheduleEmbeds(event, positions),
    },
  });
}
...
export async function runDiscordDispatch({ now = new Date(), force = false } = {}) {
  ...
  if (hourGate) {
    ...
    // 4. Rozpiska obsady N dni przed (domyślnie 1 — "ostateczny harmonogram
