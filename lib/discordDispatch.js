...
export async function announceEvent(supabase, eventId, { force = false } = {}) {
  ...
  // Było "Zapisy na wydarzenie: URL" — zmienione, bo strona eventu nie ma już
  // formularza zapisu (funkcja wyłączona 2026-09-03), więc admin przypisuje
  // kontrolerów bezpośrednio zamiast czekać na zgłoszenia.
  const cta = url ? `Szczegóły: ${url}` : undefined;
  ...
  return sendOnce({
    ...
    payload: {
      mentionRole: true,
      content: cta,
      embeds: [buildEventEmbed(event, { variant: 'announce' })],
    },
  });
}
...
export async function remindEvent(supabase, eventId, daysBefore, { force = false } = {}) {
  ...
  const cta = [
    ...
    url ? `Szczegóły / rozpiska: ${url}` : null,
  ]
    .filter(Boolean)
    .join('\n');
  ...
  return sendOnce({
    ...
    payload: {
      mentionRole: true,
      content: cta,
      embeds: [buildEventEmbed(event, { variant: 'reminder' })],
    },
  });
}
...
export async function sendSchedule(supabase, eventId, { force = false, imageBase64 = null } = {}) {
  ...
  const { data: rows, error } = await supabase
    ...
    .select(
      'id, time_start, time_end, is_free, positions(callsign, type), controllers:controllers!event_assignments_controller_id_fkey(name, cid, discord_id), student:controllers!event_assignments_student_id_fkey(name, cid, discord_id)'
    )
    .eq('event_id', eventId);
  if (error) throw new Error(error.message);
  ...
  const positions = Array.from(byPosition.values())
    ...
    .map((p) => {
      ...
      return {
        callsign: p.callsign,
        // Prawdziwy wiersz "- - - FREE - - -" (it.is_free) — bez studenta,
        // bez próby oznaczenia kogokolwiek (who() zwraca '—' dla braku
        // kontrolera, ale to wyglądałoby jak brak danych, nie świadoma
        // decyzja admina).
        lines: p.items.map((it) => {
          if (it.is_free) return `\`${hhmm(it.time_start)}–${hhmm(it.time_end)}\` - - - FREE - - -`;
          // Uczeń idzie w spoilerze (`||…||`) — prośba admina, żeby wyróżniał
          // się w rozpisce. Oznaczenie w środku spoilera DZIAŁA: Discord
          // ukrywa treść do kliknięcia, ale powiadomienie i tak wysyła, więc
          // uczeń dostaje pinga tak samo jak kontroler na pozycji.
  ...
  return sendOnce({
    ...
    payload: {
      ...
      // Linki do strony eventu i do rozpiski na żywo NIE lecą już jako
      // gołe URL-e w treści wiadomości (prośba admina, 2026-09-03 — zbyt
      // długo się to robiło) — zostają tylko w embedzie: tytuł embeda linkuje
      // do strony eventu (embed.url, niżej), a rozpiska na żywo ma własne
      // pole "ONLINE SCHEDULE" z linkiem schowanym pod tekstem (patrz
      // buildScheduleEmbeds w lib/discord.js).
      content: [
...
export async function runDiscordDispatch({ now = new Date(), force = false } = {}) {
  ...
  //    "event jest pojutrze"). Wcześniej był tu też wcześniejszy blok
  //    przypomnień o zapisach — wyłączony niżej razem z całą funkcją zapisów.
  if (hourGate) {
    ...
    // Przypomnienia o zapisach WYŁĄCZONE (2026-09-03, na prośbę admina) —
    // funkcja zapisów na eventy jest niepotrzebna na obecnym etapie rozwoju
    // aplikacji, więc automat przestał zapraszać do formularza, którego już
    // nie ma. remindSignups() zostaje w kodzie nietknięte (na wypadek
    // przywrócenia funkcji), po prostu nic już go nie wywołuje.
