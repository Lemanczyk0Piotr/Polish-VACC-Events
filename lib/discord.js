...
function mentionBlock(mentionRole) {
  ...
  return { content: undefined, allowed_mentions: { parse: [] } };
}
...
export async function postToDiscord(target, { embeds = [], mentionRole = false, content, file } = {}) {
  const url = webhookFor(target);
  if (!url) return { ok: false, error: `Brak webhooka dla kanału "${target}" w zmiennych środowiskowych.` };

  const mention = mentionBlock(mentionRole);
  const body = {
...
export function buildScheduleEmbeds(event, positionsWithShifts, { withImage = false } = {}) {
  const start = eventStart(event);
  const url = eventUrl(event.id);
  const chunks = [];
  ...
  return chunks.map((chunk, idx) => {
    ...
    if (idx === chunks.length - 1) {
      embed.footer = { text: 'Polish VACC · rozpiska obsady' };
