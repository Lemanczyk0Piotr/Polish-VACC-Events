...
// Publiczna, samoaktualizująca się rozpiska (pages/events/[id]/schedule.js) —
// dostępna dla każdego, kto ma link, bez logowania. Dołączana do rozpiski
// wysyłanej na Discorda jako osobne pole embeda "ROZPISKA ONLINE" (prośba
// admina, 2026-09-03), bo `eventUrl()` powyżej prowadzi do strony eventu, na
// której zwykły widz NIE widzi już wykresu Gantta (panel z nim jest
// admin-only) — bez tego linku Discord odsyłałby donikąd.
export function scheduleUrl(eventId) {
  const base = siteUrl();
  return base ? `${base}/events/${eventId}/schedule` : null;
}

// Role, które można pingnąć. `DISCORD_ROLE_ID` to domyślna rola wszystkich
// kontrolerów PLVACC; pozostałe są opcjonalne — jeśli zmienna jest pusta, rola
// po prostu nie pojawia się do wyboru w panelu.
export const PING_ROLES = [
  { key: 'controllers', env: 'DISCORD_ROLE_ID', label: 'PLVACC Controller', isDefault: true },
  { key: 'c1', env: 'DISCORD_ROLE_ID_C1', label: 'C1' },
  { key: 's3', env: 'DISCORD_ROLE_ID_S3', label: 'S3' },
  { key: 's2', env: 'DISCORD_ROLE_ID_S2', label: 'S2' },
  { key: 'c3', env: 'DISCORD_ROLE_ID_C3', label: 'C3' },
  { key: 'ins', env: 'DISCORD_ROLE_ID_INS', label: 'INS' },
];

export function roleIdFor(key) {
  const role = PING_ROLES.find((r) => r.key === key);
  if (!role) return null;
  const id = (process.env[role.env] || '').trim();
  return id || null;
}

// Do panelu /discord: które role da się pingnąć (bez zdradzania ich ID).
export function configuredRoles() {
  return PING_ROLES.map((r) => ({
    key: r.key,
    label: r.label,
    isDefault: Boolean(r.isDefault),
    configured: Boolean(roleIdFor(r.key)),
  }));
}

// `mentionRole: true` = domyślna rola kontrolerów (zgodność z wcześniejszym
// kodem). `mentionRoleKeys: ['c1','s3']` = konkretne role z PING_ROLES.
// `mentionUsers` to ID-ki użytkowników (controllers.discord_id).
//
// Discord wymaga wypisania wszystkiego w allowed_mentions — bez tego `<@&id>`
// czy `<@id>` w treści wyświetli się jako wzmianka, ale nikogo nie powiadomi.
// Ustawiamy to jawnie także po to, żeby treść eventu nie mogła przypadkiem
// pingnąć @everyone.
function mentionBlock(mentionRole, mentionUsers = [], mentionRoleKeys = null) {
  const keys = mentionRoleKeys || (mentionRole ? ['controllers'] : []);
  const roleIds = [...new Set(keys.map(roleIdFor).filter(Boolean))];
  ...
  if (roleIds.length > 0) {
    ...
    return { content: roleIds.map((id) => `<@&${id}>`).join(' '), allowed_mentions: allowed };
  }
...
export async function postToDiscord(
  target,
  { embeds = [], mentionRole = false, mentionRoleKeys = null, mentionUsers = [], content, file } = {}
) {
  const url = webhookFor(target);
  if (!url) return { ok: false, error: `Brak webhooka dla kanału "${target}" w zmiennych środowiskowych.` };

  const mention = mentionBlock(mentionRole, mentionUsers, mentionRoleKeys);
  const body = {
...
export function buildEventEmbed(event, { variant = 'announce', signupCount = null } = {}) {
  ...
  // Przypomnienie ma być krótkie (prośba usera): bez przeklejonego opisu
  // wydarzenia i bez licznika zapisów — sam tytuł, termin i linki. Pełny opis
  // jest w ogłoszeniu i na stronie eventu.
  if (!isReminder && typeof signupCount === 'number') {
    fields.push({
      name: 'Zapisy',
  ...
  const embed = {
    ...
    description: isReminder ? undefined : clamp(event.notes, 3500),
...
export function buildScheduleEmbeds(event, positionsWithShifts, { withImage = false, remarks = '' } = {}) {
  const start = eventStart(event);
  const url = eventUrl(event.id);
  // Discord pozwala na max 25 pól na embed. Pierwszy chunk dostaje jedno
  // dodatkowe pole niżej ("ROZPISKA ONLINE"), więc tnie się po 24 pozycjach,
  // żeby zostawić na nie miejsce — inaczej przy dokładnie 25 obsadzonych
  // pozycjach embed przekroczyłby limit i Discord odrzuciłby całą wiadomość.
  const CHUNK_SIZE = 24;
  const chunks = [];
  for (let i = 0; i < positionsWithShifts.length; i += CHUNK_SIZE) {
    chunks.push(positionsWithShifts.slice(i, i + CHUNK_SIZE));
  }
  ...
  return chunks.map((chunk, idx) => {
    ...
    if (idx === 0) {
      ...
      embed.description = [
        ...
        .join('\n');
      if (url) embed.url = url;
      // Link do zawsze aktualnej rozpiski (Gantt na żywo) — osobne pole, bo
      // embed.url powyżej prowadzi do strony eventu, nie do wykresu. Nazwa
      // pola i tekst linku po angielsku na wyraźną prośbę admina
      // (2026-09-03).
      const liveUrl = scheduleUrl(event.id);
      if (liveUrl) {
        embed.fields.push({ name: '🔗 ONLINE SCHEDULE', value: `[View live schedule here](${liveUrl})`, inline: false });
      }
    }
...
// stronie.
export function buildPeriodEmbeds({ from, to, stats, topControllers = 10 }) {
  // Ta sama zmienna co przy podsumowaniu miesiąca: serwer Discord jest
  // wewnętrzny, więc domyślnie pokazujemy nazwiska, ale
  // DISCORD_SUMMARY_SHOW_NAMES=0 przełącza cały ranking na same CID-y.
  const showNames = (process.env.DISCORD_SUMMARY_SHOW_NAMES || '1') !== '0';
  const medals = ['🥇', '🥈', '🥉'];
  const controllerLines = stats.controllers.slice(0, topControllers).map((c, i) => {
    const who = !showNames
      ...
      : c.name && c.cid
        ...
        : c.name || c.cid || '—';
    const prefix = medals[i] || `**${i + 1}.**`;
    return `${prefix} ${who} — **${fmtDuration(c.minutes)}** (${c.eventCount})`;
  });

  const eventLines = stats.events.map((e) => {
  ...
  const typeLines = stats.types.map(
    (ty) => `**${ty.type}** — ${fmtDuration(ty.minutes)} (${ty.positionCount} różnych poz.)`
  );

  // Lista wydarzeń jedzie w OPISIE drugiego embeda, nie w polach.
  // Powód: pole embeda ma limit 1024 znaków, więc kilkanaście eventów trzeba
  // było ciąć na "Wydarzenia (cd.)" — user nie chce tego podziału. Opis ma
  // limit 4096 znaków, czyli ~60 wydarzeń w jednym ciągu.
  const EVENTS_LIMIT = 3900;
  const shownEvents = [];
  let len = 0;
  for (const line of eventLines) {
    if (len + line.length + 1 > EVENTS_LIMIT) break;
    shownEvents.push(line);
    len += line.length + 1;
  }
  const overflow = eventLines.length - shownEvents.length;

  const first = {
    title: clamp(`PODSUMOWANIE OKRESU · ${from} → ${to}`, 250),
    description: [
      `Eventy: **${stats.eventCount}**`,
      `Łączny online wszystkich kontrolerów: **${fmtDuration(stats.totalMinutes)}**`,
      `Kontrolerzy którzy wzięli udział: **${stats.controllerCount}** · pozycji online: **${stats.positionCount}**`,
    ].join('\n'),
    color: 0xd32f2f,
    fields: controllerLines.length
      ? [{ name: '🏆 Top kontrolerzy', value: clamp(controllerLines.join('\n'), 1000) || '—' }]
      : [],
  };
  const base = siteUrl();
  if (base) first.url = `${base}/stats`;

  const second = {
    color: 0xd32f2f,
    description: clamp(
      ['**📅 Events**', ...shownEvents, overflow > 0 ? `…i ${overflow} więcej` : null]
        .filter(Boolean)
        .join('\n'),
      4000
    ),
    fields: typeLines.length
      ? [{ name: '🎧 Typy pozycji', value: clamp(typeLines.join('\n'), 1000) || '—' }]
      : [],
