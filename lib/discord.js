...
// stronie.
export function buildPeriodEmbed({ from, to, stats, topControllers = 10 }) {
  ...
  // WSZYSTKIE wydarzenia z okresu, nie tylko kilkanaście — user wyraźnie o to
  // poprosił. Jedno pole embeda ma limit 1024 znaków, więc listę tniemy na
  // kolejne pola. Cały embed ma limit ~6000 znaków, dlatego rezerwujemy na
  // wydarzenia najwyżej 4 pola; reszta (przy naprawdę długim okresie) zostaje
  // podsumowana licznikiem.
  const MAX_EVENT_FIELDS = 4;
  const chunks = [];
  let current = [];
  let currentLen = 0;
  let overflow = 0;
  for (const line of eventLines) {
    if (currentLen + line.length + 1 > 1000) {
      ...
      current = [];
      currentLen = 0;
      if (chunks.length >= MAX_EVENT_FIELDS) break;
    }
    current.push(line);
    currentLen += line.length + 1;
  }
  if (current.length && chunks.length < MAX_EVENT_FIELDS) chunks.push(current);
  const shown = chunks.reduce((acc, c) => acc + c.length, 0);
  overflow = eventLines.length - shown;

  chunks.forEach((chunk, i) => {
    const isLast = i === chunks.length - 1;
    const value = chunk.join('\n') + (isLast && overflow > 0 ? `\n…i ${overflow} więcej` : '');
    fields.push({ name: i === 0 ? '📅 Wydarzenia' : '📅 Wydarzenia (cd.)', value: clamp(value, 1024) || '—' });
  });
